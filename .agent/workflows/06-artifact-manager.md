---
description: 모든 에이전트가 생성한 산출물을 관리하고 메타 정보를 종합 관리하는 문서 관리자다.
---
...
2. `outputs/[프로젝트명]/artifacts/artifact-index.md`를 생성/업데이트한다.
   - 요청 ID별 산출물 목록
   - 각 산출물의 버전 정보
   - 최종 수정일 및 작성자
3. `outputs/[프로젝트명]/artifacts/metadata.json`을 생성/업데이트한다.
   - 프로젝트 메타 정보
   - 단계별 완료 상태
   - 품질 지표
4. `outputs/[프로젝트명]/artifacts/changelog.md`를 업데이트한다.
...
7. ADR(Architecture Decision Record)을 작성한다.
   - `outputs/[프로젝트명]/adr/[NNN]-[decision-title].md`
   - Context, Decision, Consequences, Alternatives
8. 최종 산출물을 Git Repository에 커밋한다.
   ```bash
   git add outputs/ && git commit -m "outputs: [프로젝트명] 산출물 최종 정리 (v[버전])"
   ```
...
산출물은 `outputs/[프로젝트명]/` 하위에 단계별로 관리되며, 버전 관리와 변경 이력 추적이 필수야.
...
## Phase 2: 산출물 인덱스 생성
1. `outputs/[프로젝트명]/artifacts/artifact-index.md` 작성
2. 디렉토리 구조 정리

## Phase 3: 메타데이터 생성
1. `outputs/[프로젝트명]/artifacts/metadata.json` 작성
2. 품질 지표 계산
...
## Phase 5: 변경 이력 관리
1. `outputs/[프로젝트명]/artifacts/changelog.md` 업데이트
...
다음 산출물들을 정리해주세요:
- outputs/상품관리/01-requirements/
- outputs/상품관리/02-analysis/
- outputs/상품관리/03-design/
